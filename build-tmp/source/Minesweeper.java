...
public void artillery() {
    if (clickCounter % 3 == 0) {
        ...
        ArrayList <MSButton> tempA = new ArrayList <MSButton>();

        if (isValid(r - 1, c - 1)) {tempA.add(buttons[r - 1][c - 1]);}
        if (isValid(r - 1, c)) {tempA.add(buttons[r - 1][c]);}
        if (isValid(r - 1, c + 1)) {tempA.add(buttons[r - 1][c + 1]);}
        if (isValid(r, c - 1)) {tempA.add(buttons[r][c - 1]);}
        if (isValid(r, c + 1)) {tempA.add(buttons[r][c + 1]);}
        if (isValid(r + 1, c - 1)) {tempA.add(buttons[r + 1][c - 1]);}
        if (isValid(r + 1, c)) {tempA.add(buttons[r + 1][c]);}
        if (isValid(r + 1, c + 1)) {tempA.add(buttons[r + 1][c + 1]);}
        ...
        for (MSButton t: tempA) {
            if (!craters.contains(t)) {stuck = false;}
        }
